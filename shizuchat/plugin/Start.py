...
async def set_group_language(chat: Chat):
    ...
    async for message in Client.get_chat_history(chat.id, limit=50):
...
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(Client.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"ʜᴇᴛ ʙᴀʙʏ!!\n{Client.name} ᴄʜᴀᴛʙᴏᴛ ιѕ ᴀʟɪᴠᴇ 🥀 ᴀɴᴅ ᴡᴀʀᴋɪɴɢ ғɪɴᴇ ᴡɪᴛʜ ᴀ ᴘɪɴɢ ᴏғ\n➥ `{ms}` ms\n\n<b>|| ᴍᴀᴅᴇ ᴡɪᴛʜ ❣️ ʙʏ [ʙᴀᴅ ᴍᴜɴᴅᴀ](https://t.me/{OWNER_USERNAME}) ||</b>",
