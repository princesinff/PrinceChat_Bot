...
from shizuchat.database.error import capture_err
from shizuchat.database.keyboard import ikb
from shizuchat.database.database import save_filter
...
async def unban_func(_, message: Message):
    ...
    await message.chat.unban_member(user_id)
    umention = (await shizuchat.get_users(user_id)).mention
    replied_message = message.reply_to_message
    if replied_message:
        message = replied_message
...
async def promoteFunc(_, message: Message):
    ...
    bot = (await shizuchat.get_chat_member(message.chat.id, shizuchat.id)).privileges
...
@shizuchat.on_callback_query(filters.regex(r"^check_powers_(\d+)"))
async def check_powers_callback(_, query: CallbackQuery):
    user_id = int(query.data.split("_")[2])
    bot = (await shizuchat.get_chat_member(query.message.chat.id, shizuchat.id)).privileges
    user_privileges = (await shizuchat.get_chat_member(query.message.chat.id, user_id)).privileges
...
# Toggle admin power
@shizuchat.on_callback_query(filters.regex(r"^toggle_(.+)_(\d+)"))
async def toggle_power_callback(_, query: CallbackQuery):
    ...
    current_privs = (await shizuchat.get_chat_member(query.message.chat.id, user_id)).privileges
...
@shizuchat.on_callback_query(filters.regex(r"^close"))
async def close_callback(_, query: CallbackQuery):
    await query.message.delete()

@shizuchat.on_callback_query(filters.regex(r"^back"))
...
async def purgeFunc(_, message: Message):
    ...
    for message_id in range(
        ...
        if len(message_ids) == 100:
            await shizuchat.delete_messages(
                chat_id=chat_id,
...
async def demote(_, message: Message):
    ...
    try:
        member = await shizuchat.get_chat_member(message.chat.id, user_id)
...
@shizuchat.on_callback_query(filters.regex(r"unpin_(yes|no)"))
async def callback_query_handler(_, query: CallbackQuery):
    if query.data == "unpin_yes":
        await shizuchat.unpin_all_chat_messages(query.message.chat.id)
...
@shizuchat.on_callback_query(filters.regex("unwarn_") & ~BANNED_USERS)
...
async def ban_members(chat_id, user_id, bot_permission, total_members, msg):
    ...
    while failed_count <= 30:
        async for member in shizuchat.get_chat_members(chat_id):
            if failed_count > 30:
                break  # Stop if failed bans exceed 30
            
            try:
                if member.user.id != user_id and member.user.id not in SUDOERS:
                    await shizuchat.ban_chat_member(chat_id, member.user.id)
                    banned_count += 1
...
async def ban_all(_, msg):
    ...
    bot = await shizuchat.get_chat_member(chat_id, BOT_ID)
    bot_permission = bot.privileges.can_restrict_members
    
    if bot_permission:
        total_members = 0
        async for _ in shizuchat.get_chat_members(chat_id):
            total_members += 1
