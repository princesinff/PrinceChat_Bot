...
from shizuchat import LOGGER
from shizuchat import shizuchat
from shizuchat.database.permissions import member_permissions
...
from shizuchat.welcome.antispam_db import GBan
from shizuchat.welcome.greetings_db import Greetings
from shizuchat.welcome.supports import get_support_staff
from shizuchat.welcome.cmd_senders import send_cmd
from shizuchat.welcome.kbhelpers import ikb
from shizuchat.welcome.msg_types import Types, get_wlcm_type
from shizuchat.welcome.parser import escape_markdown, mention_html
from shizuchat.welcome.string import (build_keyboard, escape_invalid_curly_brackets,
                                 parse_button)
import config
...
@shizuchat.on_message(filters.service & filters.group, group=59)
...
@shizuchat.on_chat_member_updated(filters.group, group=69)
async def member_has_joined(c: shizuchat, member: ChatMemberUpdated):
...
@shizuchat.on_chat_member_updated(filters.group, group=99)
async def member_has_left(c: shizuchat, member: ChatMemberUpdated):
...
@adminsOnly("can_restrict_members")
async def welcome(c: shizuchat, m: Message):
    db = Greetings(m.chat.id)
    status = db.get_welcome_status()
    oo = db.get_welcome_text()
    args = m.text.split(" ", 1)
...
@adminsOnly("can_restrict_members")
async def goodbye(c: shizuchat, m: Message):
    db = Greetings(m.chat.id)
    status = db.get_goodbye_status()
    oo = db.get_goodbye_text()
    args = m.text.split(" ", 1)
